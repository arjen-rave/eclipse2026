...
- [x] J — Camera aiming improvements — complete, see "Milestone J" above
  ...
  - [x] J follow-up #4 — user initially reported the reverted file-input
        mechanism *also* did nothing (same symptom as the failed intent-link
        attempts); root cause not identified before the user confirmed it
        actually works now (never fully explained — possibly resolved by the
        close/reopen cycle itself, or was environment-specific — not chased
        further once working). Added an always-visible version tag
        (bottom-right, muted, hardcoded string bumped by hand alongside
        `CACHE_NAME` each deploy — `index.html`/`sw.js` don't share a JS scope
        to derive it automatically) so the user can confirm which build is
